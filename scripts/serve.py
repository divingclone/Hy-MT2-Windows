...
def main():
    ...
    parser.add_argument('--variant', choices=('auto', 'optimized', 'baseline', 'portable'), default='auto')
    ...
    binary_dir = ROOT/'build'/args.variant/'bin' if args.variant != 'auto' else None
    model = args.model
    if args.variant == 'baseline' and model is None:
        model = str(ROOT/'models/Hy-MT2-1.8B-Q4_K_M.gguf')
    ...
    command = [str(exe), '-m', config['model'], '--alias', 'hy-mt2', '--host', '127.0.0.1',
               ...
               '--repeat-penalty', '1.05', '--repeat-last-n', str(max(4096, args.context)), '--metrics', '--no-warmup']
    if args.backend_sampling:
        command.append('--backend-sampling')
    config.update(command=command, batch=batch, port=args.port)
    (ROOT/'results'/f'{args.label}.config.json').write_text(json.dumps(config, ensure_ascii=False, indent=2), encoding='utf-8')
