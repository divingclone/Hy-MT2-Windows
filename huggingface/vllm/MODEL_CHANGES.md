...
The original Apache-2.0 license text is preserved in `LICENSE`.
...
- GPTQ INT4 W4A16, compressed-tensors format, symmetric weight quantization with
  ...
  source hashes and output hashes are recorded in `quantization.json` in the INT4 repository.
...
Every checkpoint file's size and SHA-256 are recorded in `checksums.json`. KV cache quantization is selected at
