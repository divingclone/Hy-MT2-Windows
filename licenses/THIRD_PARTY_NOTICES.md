...
| CPython 3.12.13, standalone build 20260728 | Original `runtime/python/LICENSE.txt`, plus `python-3.12-license.html` with the Python documentation's additional component notices. The latter is a documentation snapshot for the 3.12 series, not proof of every binary component version. Build distribution: https://github.com/astral-sh/python-build-standalone/releases/tag/20260728 |
...
| SQLite | `sqlite-PUBLIC-DOMAIN.html`, the project's public-domain statement. The bundled DLL reports version 3.53.1: https://www.sqlite.org/copyright.html |
...
| Microsoft Visual C++ Runtime | Unmodified runtime DLLs, the official end-user license (`msvc-runtime-license.docx` plus extracted `.txt`), and the Visual Studio 2022 redistribution list (`msvc-redistribution.html`). Redistribution authority comes from an applicable licensed developer product and its REDIST terms, **not** from the end-user runtime license alone: https://learn.microsoft.com/en-us/visualstudio/releases/2022/redistribution |
...
Downloaded and extracted notice source URLs/paths and hashes are in
`SOURCES.json`. The package's manifest independently hashes the actual files
