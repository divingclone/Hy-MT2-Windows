...
- NVFP4: quantized a BF16 GGUF from the Unsloth conversion of the same Tencent model using translation-template calibration from the fixed OPUS-100 English/Chinese validation split. Local scale search minimizes input-column-diagonal imatrix-weighted squared error while retaining the reference scale as a candidate. The result is repacked for joined projections. Calibration and quality evaluation use disjoint validation/test inputs; no low-bit model is dequantized to create these weights.
- The distributed NVFP4 payload uses local scale search. Full scale search is an offline research option and is not the distributed model. Calibration sources, hashes, importance-matrix settings and the distributed payload checksum are recorded in provenance.json.
