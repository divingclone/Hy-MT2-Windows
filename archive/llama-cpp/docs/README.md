...
量化劣化分析另见 [原始 BF16 输出参考评测](docs/TEACHER_FIDELITY.md)：以未量化模型的译文为参考，比较 4 位权重、激活精度、8 位 KV 和当前 llama.cpp，并结合固定参考前缀的 token 概率。文本一致性不等于语义正确率。

新增 [Windows 原生 vLLM 8 位 KV 测试](docs/VLLM_KV.md)：同一 RTX 5090、**4 位 NVFP4、32/256 并发**，预分词、同进程预热的核心对照中，vLLM INT8 KV 吞吐分别为 llama.cpp Q8 KV 的 **1.37 倍 / 4.63 倍**。相对 vLLM BF16 KV，INT8 在两档并发分别节省约 **26% / 32%** 的整任务显存峰值增量，吞吐变化 **−6.0% / +5.9%**；留出质量分数接近，不能宣称普遍无损。复现入口 `scripts/benchmark_vllm_kv.py`，独立服务入口 `scripts/serve_vllm.py` 默认 INT8 KV；[原 16 位 KV、HTTP 与权重量化对照](docs/VLLM_MIGRATION.md) 单独保留。使用独立环境和社区 Windows 构建，桌面端与现有运行包仍使用 llama.cpp。
